...
// prettier-ignore
const STATE_MESSAGES: { [k in keyof typeof PipelineState]: [color: string, status: string] } = {
...
async function handler(event: any, _context: any, callback?: LambdaCallback): Promise<LambdaResponse> {
  ...
  try {
    if (isSupportedHTTPLambdaRequest(event)) {
      ...
      if (event.headers["Authorization"] !== `Bearer ${API_SECRET}`) return sendResponse("Unauthorized", 401);
    }
    ...
    if (!pipelineState) return sendResponse("Unsupported state", 400);
    ...
    if (pipelineState === PipelineState.started) {
      ...
      const endingStates = [PipelineState.finished, PipelineState.stopped, PipelineState.failed];
